...
def xyz2uvd(joints, intrinsic):
    joints = np.matmul(intrinsic, joints)
    joints[0, :] /= joints[2, :] 
    joints[1, :] /= joints[2, :] 
    return joints

def kp_to_bbox_param(kp):
    """
    ...
    min_pt = np.min(kp[:, :2], axis=0)
    max_pt = np.max(kp[:, :2], axis=0)
    person_height = np.linalg.norm(max_pt - min_pt)
    if person_height < 0.5:
        return
    center = (min_pt + max_pt) / 2.
    w = h = person_height * 1.1
    bbox = np.array([center[0], center[1], w, h])
    return bbox
    

def get_dataset_path(dataset):
    if dataset == "mpii":
        train_folder = "/data/MPII/crop_image_aspect"
        img_path_list = glob.glob(osp.join(train_folder, "*.jpg"))
        num_train = len(img_path_list)
        print("MPII train num: {}".format(num_train))
        
    elif dataset == "human3.6m":
        root = "/data/h36m/images"
        img_path_list = glob.glob(osp.join(root, "*.jpg"))
        num_test = len(img_path_list)
        print("MPII test num: {}".format(num_test))
