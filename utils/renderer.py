...
def simple_renderer(rn,
    ...
    rn.vc = LambertianPointLight(
        ...
        vc=albedo,
        light_color=np.array([1, 1, 1])) # light_color=np.array([1, 1, 1]
    ...
    rn.vc += LambertianPointLight(
        ...
        vc=albedo,
        light_color=np.array([.7, .7, .7])) # light_color=np.array([.7, .7, .7]
...
def get_original(verts, cam, joints, img_size):
    img_size = 224
    undo_scale = 1
